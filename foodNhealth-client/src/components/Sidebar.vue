        ...
        <template v-for="(item, index) in navItems">
          ...
          <template v-else>
            <template v-if="item.children">
              ...
              <SidebarNavDropdown :name="item.name" :url="item.url" :icon="item.icon" v-if="checkPermissionsByUrl(item.url)">
                  ...
                  <template v-if="childL1.children">
                    ...
                    <SidebarNavDropdown :name="childL1.name" :url="childL1.url" :icon="childL1.icon">
                      <li class="nav-item" v-for="(childL2, index) in childL1.children">
                        <SidebarNavLink :name="childL2.name" :url="childL2.url" :icon="childL2.icon" :badge="childL2.badge" :variant="item.variant" v-if="checkPermissionsByUrl(item.url)"/>
                      </li>
                    </SidebarNavDropdown>
                  </template>
                  <template v-else>
            ...
            <template v-else>
              <SidebarNavItem :classes="item.class"  v-if="checkPermissionsByUrl(item.url)">
